...
def render_footer() -> None:
    st.markdown(
        ...
        <div class="aegis-footer">
            ...
            the European Commission, the AI Office, or any Irish authority. It
            does not store your inputs; data is held in your session only and is
            sent to a third-party language model for processing.
...
PRIVACY_WARNING = (
    ...
    'person). Your description is sent to a third-party language model for '
    'processing.</div>'
)
...
def screen_inventory() -> None:
    ...
    st.write(
        "Tell Aegis what the system does in plain language. One system at a "
        "time. The next steps classify its risk tier under the EU AI Act and "
        "list the obligations that apply."
    )
...
def screen_classification() -> None:
    ...
    if needs_review:
        st.markdown(
            """
            <div class="aegis-disclaimer" style="margin-top:1rem;">
                This classification is flagged for human review. Treat it as a
                starting point and confirm the tier with qualified counsel.
            </div>
            """,
            unsafe_allow_html=True,
        )
...
def screen_ask() -> None:
    ...
    st.write(
        "Ask a question about the EU AI Act. Answers are grounded in and cite "
        "the actual legislation. Aegis declines to answer if the retrieved text "
